...
ADD CONSTRAINT customer_address_check CHECK (address~*'^(accesso|arco|belvedere|borgo|campo|canale|cascina|cavone|cavalcavia|contrada|corso|cortile|costa|discesa|fondo|galleria|frazione|isola|lido|litoranea|lungo|
												 ...
												 spiaggia|spiazzo|strada|stradale|stretto|stretta|strettoia|terrazza|traversa|via|vicoletto|vico|villaggio|viuzza)( )[ A-Za-z0-9]+(, )[ A-Z0-9]+(, )[0-9]{5}(, )[A-Za-z]+(, )[ A-Za-z]+$' );
...
ALTER TABLE Customer
ADD CONSTRAINT customer_email_check CHECK (email ~* '^[A-Za-z0-9._%-]+(@)[A-Za-z0-9.-]+[.][A-Za-z]+$'::text); -- OK

ALTER TABLE Customer
ADD CONSTRAINT customer_cf_check CHECK (cf ~* '^[a-zA-Z]{6}[0-9]{2}[abcdehlmprstABCDEHLMPRST]{1}[0-9]{2}([a-zA-Z]{1}[0-9]{3})[a-zA-Z]{1}$'); -- OK

--Constraint closing_days shop
ALTER TABLE Shop 
ADD CONSTRAINT shop_closing_days_check CHECK (closing_days ~* '^((lunedì|lunedi|martedì|martedi|mercoledì|mercoledi|giovedì|giovedi|venerdì|venerdi|sabato|domenica)(, (?!$)|$))*$'); -- OK

--Constraint working_hours shop
ALTER TABLE Shop
ADD CONSTRAINT shop_working_hours_check CHECK (working_hours ~* '^([0-2]{1}[0-9]{1})(:)([0-5]{1}[0-9]{1})(-)([0-2]{1}[0-9]{1})(:)([0-5]{1}[0-9]{1})$'); -- OK

--Constraint email shop
ALTER TABLE Shop
ADD CONSTRAINT shop_email_check CHECK (email ~* '^[A-Za-z0-9._%-]+(@foodoverflow.it)$'); -- OK

--Constraint working_hours rider
ALTER TABLE Rider 
ADD CONSTRAINT rider_working_hours_check CHECK (working_hours ~* '^([0-2]{1}[0-9]{1})(:)([0-5]{1}[0-9]{1})(-)([0-2]{1}[0-9]{1})(:)([0-5]{1}[0-9]{1})$'); -- OK

--Constraint codice fiscale rider
ALTER TABLE Rider
ADD CONSTRAINT rider_cf_check CHECK (cf ~* '^[a-zA-Z]{6}[0-9]{2}[abcdehlmprstABCDEHLMPRST]{1}[0-9]{2}([a-zA-Z]{1}[0-9]{3})[a-zA-Z]{1}$'); -- OK

--Constraint cellphone customer
ALTER TABLE Customer
ADD CONSTRAINT customer_cellphone_check CHECK (cellphone ~ '^[0-9]{10}$'); -- OK

--Constraint cellphone rider
ALTER TABLE Rider
ADD CONSTRAINT rider_cellphone_check CHECK (cellphone ~ '^[0-9]{10}$'); -- OK
