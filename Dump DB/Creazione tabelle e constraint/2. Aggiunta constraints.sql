...
Controlla che l' indirizzo abbia il seguente formato: "<denominazione urbanistica>, <indirizzo>, <numero civico>, <CAP>, <Comune>, <Provincia>". Non e' case sensitive.
...
ALTER TABLE Shop
Controlla che l' email abbia dominio "foodoverflow.it". Non e' case sensitive.
ADD CONSTRAINT shop_email_check CHECK (email ~* '^[A-Za-z0-9._%-]+(@foodoverflow.it)$');

-- Controllo codice fiscale del cliente e del rider.
ALTER TABLE Customer
Controlla che il codice fiscale sia scritto correttamente. E' case sensitive.
...
ALTER TABLE Shop 
Controlla che l' attributo closing_days sia una stringa formata da giorni della settimana separati da virgola-spazio, senza ripetizioni. Non e' case sensitive.
...
ALTER TABLE Shop
Controlla che la fascia oraria lavorativa sia del formato (HH:MM-HH:MM)
...
Controlla che il numero di cellulare abbia 10 cifre numeriche.
ALTER TABLE Customer
...
Controlla che un rider sia associato al piu' a 3 consegne.
...
Controlla che la quantita' di un meal nel carrello sia maggiore di 0.
...
Controlla che lo stato dell' ordine sia uno tra quelli riportati di sotto nell' espressione regolare. Non e' case sensitive.
...
Controlla che la categoria di un meal sia una tra quelle riportate di sotto nell' espressione regolare. Non e' case sensitive.
ALTER TABLE Meal
...
Controlla che l' allergene sia uno tra quelli riportati di sotto nell' espressione regolare.Non e' case sensitive.
...
Controlla che il genere sia "M" oppure "F". Non e' case sensitive.
ALTER TABLE Rider
