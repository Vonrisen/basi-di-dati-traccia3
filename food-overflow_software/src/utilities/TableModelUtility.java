...
public class TableModelUtility {
	...
	public void fillFieldsFromJTable(ShopRiderFrame shop_rider_frame) {
		...
		if (!table.getSelectionModel().isSelectionEmpty()) {
			...
			if(table.getModel().getValueAt(row, 6).toString().equals("M"))
				shop_rider_frame.getGenderCB().setSelectedItem("Maschio");
			else
				shop_rider_frame.getGenderCB().setSelectedItem("Femmina");
