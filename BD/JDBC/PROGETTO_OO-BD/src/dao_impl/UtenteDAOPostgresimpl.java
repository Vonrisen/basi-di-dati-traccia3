...
public class UtenteDAOPostgresimpl implements UtenteDAO {
	   ...
	   private PreparedStatement getUtenteByNomePS, insertIntoPersonaPS, insertIntoUtentePS, utenteJoinPersonaPS, validaLoginUtentePS;
	    ...
	    public UtenteDAOPostgresimpl(Connection connection) throws SQLException {
	        ...
	        validaLoginUtentePS = connection.prepareStatement("SELECT * FROM Utente WHERE email =? AND password =?");
	    }
	    ...
	    public List<Utente> getUtenteByNome(String name) throws SQLException
	    {
	        ...
	        while(rs.next())
	        {
	            Utente s = new Utente(rs.getString("cf"), rs.getString("nome"), rs.getString("cognome"), rs.getString("residenza"), rs.getDate("data_n"),
	            					  rs.getString("luogo_n"), rs.getString("sesso"), rs.getString("cellulare"), rs.getString("cod_utente"), rs.getString("email"), rs.getString("password"), rs.getString("flag"));
	            lista.add(s);
	        }
	        rs.close();
		...
		public boolean validaLoginUtente(String email, String password) throws SQLException {
			...
			validaLoginUtentePS.setString(1,email);
			validaLoginUtentePS.setString(2,password);
			ResultSet rs = validaLoginUtentePS.executeQuery();
			boolean valida = rs.next();
			rs.close();
			validaLoginUtentePS.close();
			return valida;
		}
